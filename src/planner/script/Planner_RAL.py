...
class Planner:
    def __init__(self):
        ...
        self.end_time = 0
        mat = io.loadmat(
            '/home/bear/Github/habitat-lab/src/planner/script/pos.mat')
    ...
    def acquire_transition(self, state_index):
        ...
        if not state_actions:
            state = np.unravel_index(state_index, (self.old_height, self.old_width))
            self.P[state_index] = {}
            for action in range(self.nA):
                ...
                next_frontier_states = np.ravel_multi_index(next_states_frontier.T, (3, self.old_height, self.old_width))
                next_obstacle_states = np.ravel_multi_index(next_states_obstacle.T, (3, self.old_height, self.old_width))
