...
class Planner:
    def __init__(self):
        ...
        self.classes = ['towel', 'objects', 'lighting', 'stool', 'counter',
                        ...
                        'banana', 'cheezit']
        self.target_class = ['towel', 'stool', 'tv_monitor']
    ...
    def construct_dfa(self):
        self.dfa = DFA(0, ['1', '2', '3', 'D'], final_states={1, 2, 3, 4}, goal_states={3:0.1, 2:0.9, 1:1})
        # '1' Complete task A
        # '2' Not complete task A but complete task B or C
        # '3' Not complete task A, B, C, but complete task D
        # '4' Compelete No task

        # D hit obstacle

        self.dfa.add_transition('4', 0, 0)
        self.dfa.add_transition('1', 0, 1)
        self.dfa.add_transition('2', 0, 2)
        self.dfa.add_transition('3', 0, 3)

        self.dfa.add_transition('D', 0, 4)

    def label_function_prob(self, state):
        # D hit obstacle
        labels = {}

        target_class = self.target_class[0]
        target_class_index = self.classes.index(target_class)

        A = (self.old_max_class_prob.get(target_class_index, 0) > 0.9)

        if A:
            obj_id = self.old_obj_of_interest[target_class_index]
            A *= self.old_goal_prob[obj_id][state[0], state[1]]

        target_class = self.target_class[1]
        target_class_index = self.classes.index(target_class)

        B = (self.old_max_class_prob.get(target_class_index, 0) > 0.9)

        if B:
            obj_id = self.old_obj_of_interest[target_class_index]
            B *= self.old_goal_prob[obj_id][state[0], state[1]]

        target_class = self.target_class[2]
        target_class_index = self.classes.index(target_class)

        C = (self.old_max_class_prob.get(target_class_index, 0) > 0.9)

        if C:
            obj_id = self.old_obj_of_interest[target_class_index]
            C *= self.old_goal_prob[obj_id][state[0], state[1]]

        D = self.old_frontier_prob[state[0], state[1]]

        E = self.old_obstacle_prob[state[0], state[1]]

        labels['D'] = E
        labels['1'] = A*(1-E)
        labels['2'] = (1-A)*(1 -(1-B)*(1-C))*(1-E)
        labels['3'] = (1-A)*(1-B)*(1-C)*D*(1-E)
        labels['4'] = (1-A)*(1-B)*(1-C)*(1-D)*(1-E)
    ...
    def label_function(self, pos_grid):
        ...
        if self.old_obstacle_map[pos_grid[1], pos_grid[0]]:
            ...
            if A:
                obj_id = self.old_obj_of_interest[target_class_index]
                A = A and self.old_goal_map[obj_id][pos_grid[1], pos_grid[0]]
            ...
            if A:
                ...
                if B:
                    obj_id = self.old_obj_of_interest[target_class_index]
                    B = B and self.old_goal_map[obj_id][pos_grid[1], pos_grid[0]]
                ...
                if C:
                    obj_id = self.old_obj_of_interest[target_class_index]
                    C = C and self.old_goal_map[obj_id][pos_grid[1], pos_grid[0]]

                if B or C:
                    symbols.append('2')
                else:
                    if self.old_frontier_prob[pos_grid[1], pos_grid[0]]:
                        symbols.append('3')
                    else:
                        symbols.append('4')
        return symbols
    ...
    def acquire_transition(self, state):
        ...
        if not state_actions:
            ...
            for action in range(self.nA):
                    ...
                    for label in labels:
                        if labels[label] > 0:
                            ...
                            if dfa_state in self.dfa.final_states:
                                ...
                                next_dfa_state = self.dfa.get_transition(label, dfa_state)
                ...
                if not len(pnsrt):
                    print('found empty state and action pair')
                    print(next_states)
                    for i in range(3):
                        next_state = next_states[i]
                        labels = self.label_function_prob(next_state)
                        ...
                        for label in labels:
                            if labels[label] > 0:
                                ...
                                if dfa_state in self.dfa.final_states:
                                    ...
                                    next_dfa_state = self.dfa.get_transition(label, dfa_state)
                                print(i, label, dfa_state, next_dfa_state)
        ...
        while not rospy.is_shutdown():
            if self.obstacle_map is not None and not self.initial_obs and not self.mission_status:
                ...
                if self.replan:
                    ...
                    self.old_obj_of_interest = self.obj_of_interest.copy()
                    self.old_max_class_prob = self.max_class_prob.copy()
                    print('copy data')
                    # print(self.old_obj_of_interest)
                    # print(self.obj_of_interest)
                    # print(self.old_max_class_prob)
                    self.mutex.release()
                    print('mutex release')
                    ...
                    self.V = {}
                    self.P = {}
                    Vh = self.old_frontier_map*self.dfa.goal_states[3]
                    target_class = self.target_class[0]
                    target_class_index = self.classes.index(target_class)
                    A = self.old_max_class_prob.get(target_class_index, 0) > 0.9
                    ...
                    if A:
                        obj_id = self.old_obj_of_interest[target_class_index]
                        print('possible ', target_class, ' identified')
                        Vh += self.dfa.goal_states[1]*self.old_goal_map[obj_id]

                    target_class = self.target_class[1]
                    target_class_index = self.classes.index(target_class)
                    B = self.old_max_class_prob.get(target_class_index,
                                                    0) > 0.9

                    if B:
                        obj_id = self.old_obj_of_interest[target_class_index]
                        Vh += self.dfa.goal_states[2] * self.old_goal_map[
                            obj_id]
                        print('possible ', target_class, ' identified')

                    target_class = self.target_class[2]
                    target_class_index = self.classes.index(target_class)
                    C = self.old_max_class_prob.get(target_class_index,
                                                    0) > 0.9
            ...
            if self.grid_map_grey[pos_grid[1], pos_grid[0]] != 255:
                ...
                if self.goal_status:
                    ...
                    if self.dfa_state == 1:
                        ...
                        target_class_index = self.classes.index(target_class)
                        obj_id = self.old_obj_of_interest[target_class_index]
                        goal_phase = self.old_goal_phase[obj_id]

                    elif self.dfa_state == 2:
                        target_class = self.target_class[1]
                        target_class_index = self.classes.index(target_class)
                        obj_id = self.old_obj_of_interest[target_class_index]
                        goal_phase = self.old_goal_phase[obj_id]
    ...
    def find_sightings(self):
        ...
        for obj_id, obj in self.objects.items():
            ...
            for target_class in self.target_class:
                target_class_index = self.classes.index(target_class)

                if obj.class_probs[target_class_index] > self.max_class_prob.get(target_class_index, 0):
                    self.max_class_prob[target_class_index] = obj.class_probs[target_class_index]
                    self.obj_of_interest[target_class_index] = obj_id

        for target_class_index in self.obj_of_interest:
            obj_id = self.obj_of_interest[target_class_index]
        ...
        #image_message = self.bridge.cv2_to_imgmsg(sightings_map, encoding="mono8")
